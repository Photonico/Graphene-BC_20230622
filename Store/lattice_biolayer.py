...
def summarize_biolayer_lattice(directory=".", lattice_start = None, lattice_end = None, distance_start = None, distance_end = None):
...
def extract_minimum_biolayer_lattice(source_data):
...
def extract_maximum_biolayer_lattice(source_data):
    lattice_source, distance_source, free_energy_source = read_biolayer_lattice_data(source_data)
    lattice_fitted, distance_fitted, free_energy_fitted = polynomially_fit_surface(lattice_source, distance_source, free_energy_source, 3, 4000)
...
def extract_extreme_biolayer_lattice(extract_type, source_data):
    if extract_type in ("Minium", "minium", "MIN", "Min", "min"):
        return extract_minimum_biolayer_lattice(source_data)
    if extract_type in ("Maxium", "maxium", "MAX", "Max", "max"):
        return extract_maximum_biolayer_lattice(source_data)
