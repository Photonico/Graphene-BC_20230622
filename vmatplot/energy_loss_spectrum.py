#### Energy loss spectrum calculation and plotting
...
def create_matters_energy_loss(*args):
...
def plot_energy_loss_XZ_row(title, matters_list=None, unit=None, inplane_boundary=(None, None), outplane_boundary=(None, None)):
    ...
    dataset = create_matters_energy_loss(matters_list)
    subtitles = ["In-plane", "Out-of-plane"]

    # Title
    # Suptitle
    current_title = title
    fig.suptitle(f"Energy loss spectrum {current_title}", fontsize=fig_setting[3][0], y=1.00)
    ...
    for supplot_index in range(2):
        ...
        for _, data in enumerate(dataset):
            ...
            if supplot_index == 0:
                ...
                inplane_variables_full = com_energy_loss_spectrum(data[1]["density_xx_real"],data[1]["density_xx_imag"])
            ...
            # Outplane
                ...
                outplane_variables_full = com_energy_loss_spectrum(data[1]["density_zz_real"],data[1]["density_zz_imag"])
        ...
        # axis label
        if supplot_index == 0:
            ax.set_ylabel(r"Energy loss spectrum")
        if unit in ["nm", "NM"]:
