#### Extinction coefficient calculation and plotting
...
def create_matters_extinction(*args):
...
def plot_extinction_XZ_row(title, matters_list=None, unit=None, inplane_boundary=(None, None), outplane_boundary=(None, None)):
    ...
    dataset = create_matters_extinction(matters_list)
    ...
    for supplot_index in range(2):
        ...
        # axis label
        if supplot_index == 0:
            ax.set_ylabel(r"Extinction coefficient")
        if unit in ["nm", "NM"]:
...
def plot_extinction_XZ_zoom(title, matters_list=None, unit=None,
    ...
    comp_function = create_matters_extinction
