...
for y,label,color in [(.76,r"$E_C$",BLUE),(.60,r"$E_F$",FERMI),(.29,r"$E_V$","#EB731E")]:
    ...
    ax.text(.07,y,label,ha="right",va="center",fontsize=20)
...
for level,color,start,label in [(1.42,BLUE,ticks[-2],r"$\Phi_n$"),(-.87,"#EB731E",1-.08/length,r"$\Phi_p$")]:
    ...
    ax.text(1-.2/length,level/2,label,color=color,fontsize=20,va="center")
ax.text(.035,.51,r"$E_\mathrm{F}=0$",transform=ax.transAxes,color=FERMI,fontsize=16,va="bottom")
